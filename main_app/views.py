...
def send_consultation_email(request):
    if request.method == 'POST':
        ...
        patient_name = request.POST.get('patient_name')  # Get the patient's name from the AJAX request data
        print("Patient name = "+patient_name)
        # Process and send the email
        try:
          # Construct the email message
            email_message = f"{patient_name} is online and has requested for an appointment."
            print(email_message)
            # Your email sending logic here
            send_mail(
                'Patient Appointment Requested',
                 email_message,
                'jamespolkuudery@gmail.com',
                [recipient_email],
            )
            print("email sent")
